...
from StudySync.backend.app import db, login
...
class User(db.Model, UserMixin):
    ...
    firstname: so.Mapped[str] = so.mapped_column(sqla.String(64), index=True, unique=True)
    lastname: so.Mapped[str] = so.mapped_column(sqla.String(64), index=True, unique=True)
